...
export async function GET(request: NextRequest) {
  try {
    ...
    // Generate cache key
    const cacheKey = CACHE_KEYS.FILM.TRENDING(page)
    ...
    const data = await cacheWithFallback(
      ...
      CACHE_TTL.SHORT // 5 minutes for trending films (changes frequently)
    )
    ...
    return NextResponse.json(
      { error: 'Failed to fetch trending films', details: errorMessage },
      { status: 500 }
    )
  }
}